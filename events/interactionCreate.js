    ...
    if (interaction.isCommand()) {
      ...
      if (time_stamps.has(interaction.user.id)) {
        ...
        if (current_time < expiration_time) {
          ...
          const embed = new MessageEmbed().setColor("RED").setTitle(`${client.emoji.wrong} Too Fast!`).setDescription(`**You are in a cooldown! Please wait \`${time_left.toFixed(1)}\` seconds, To Use the command, \`${cmd.name}\` Again**!`).setFooter(`Powered by cyberneticsdev.tk`)
